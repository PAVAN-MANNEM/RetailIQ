  ...
  useEffect(() => {
    const runCacheCheck = () => {
      ...
      if (cacheTTL === "1h" && elapsedMs > 3600000) {
        ...
        if (elapsedMs > 1000) handleClearCache(); 
      }
    };
    ...
    <div className="mx-auto mt-6 flex max-w-[1600px] flex-col gap-8 min-w-0 pb-20">
      ...
      <Card className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl">
        <CardHeader className="flex flex-col md:flex-row md:items-start justify-between pb-2 border-b border-slate-800/50 mb-4 gap-4">
          ...
          <Button 
            variant="outline" 
            className="rounded-xl border-slate-700 bg-slate-900 text-slate-300 h-9 transition-all"
            onClick={checkFilesRealTime}
            disabled={isSyncing}
          >
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 flex flex-col gap-3">
                 <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 min-w-0">
                       <Database className={`h-4 w-4 mt-0.5 shrink-0 ${fileStatus.cleaned.ok ? 'text-emerald-400' : 'text-slate-500'}`} />
                       <span className="text-sm font-medium text-white break-all leading-tight">cleaned_superstore.csv</span>
                    </div>
                    {fileStatus.cleaned.ok ? <CheckCircle2 className="h-5 w-5 text-emerald-500 shrink-0" /> : <Server className="h-5 w-5 text-slate-500 shrink-0 animate-pulse" />}
                 </div>
                 <p className="text-xs text-slate-400">Core master dataset. Contains 51,290 chronological sales records.</p>
                 <div className="text-[11px] font-mono text-slate-500 bg-slate-950 rounded-lg p-2 mt-auto">Size: {fileStatus.cleaned.size} | Status: {fileStatus.cleaned.status}</div>
              </div>

              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 flex flex-col gap-3">
                 <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 min-w-0">
                       <HardDrive className={`h-4 w-4 mt-0.5 shrink-0 ${fileStatus.product.ok ? 'text-emerald-400' : 'text-slate-500'}`} />
                       <span className="text-sm font-medium text-white break-all leading-tight">product_search_database.json</span>
                    </div>
                    {fileStatus.product.ok ? <CheckCircle2 className="h-5 w-5 text-emerald-500 shrink-0" /> : <Server className="h-5 w-5 text-slate-500 shrink-0 animate-pulse" />}
                 </div>
                 <p className="text-xs text-slate-400">Pre-computed aggregation engine for the Products Dashboard.</p>
                 <div className="text-[11px] font-mono text-slate-500 bg-slate-950 rounded-lg p-2 mt-auto">Size: {fileStatus.product.size} | Status: {fileStatus.product.status}</div>
              </div>

              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 flex flex-col gap-3">
                 <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 min-w-0">
                       <HardDrive className={`h-4 w-4 mt-0.5 shrink-0 ${fileStatus.customer.ok ? 'text-emerald-400' : 'text-slate-500'}`} />
                       <span className="text-sm font-medium text-white break-all leading-tight">customer_search_database.json</span>
                    </div>
                    {fileStatus.customer.ok ? <CheckCircle2 className="h-5 w-5 text-emerald-500 shrink-0" /> : <Server className="h-5 w-5 text-slate-500 shrink-0 animate-pulse" />}
                 </div>
                 <p className="text-xs text-slate-400">Pre-computed RFM aggregation engine for the Customers Dashboard.</p>
                 <div className="text-[11px] font-mono text-slate-500 bg-slate-950 rounded-lg p-2 mt-auto">Size: {fileStatus.customer.size} | Status: {fileStatus.customer.status}</div>
              </div>

              <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-5 flex flex-col gap-3">
                 <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 min-w-0">
                       <HardDrive className={`h-4 w-4 mt-0.5 shrink-0 ${fileStatus.geo.ok ? 'text-emerald-400' : 'text-slate-500'}`} />
                       <span className="text-sm font-medium text-white break-all leading-tight">geo_search_database.json</span>
                    </div>
                    {fileStatus.geo.ok ? <CheckCircle2 className="h-5 w-5 text-emerald-500 shrink-0" /> : <Server className="h-5 w-5 text-slate-500 shrink-0 animate-pulse" />}
                 </div>
                 <p className="text-xs text-slate-400">Pre-computed geographic hierarchies for the Geography Dashboard.</p>
                 <div className="text-[11px] font-mono text-slate-500 bg-slate-950 rounded-lg p-2 mt-auto">Size: {fileStatus.geo.size} | Status: {fileStatus.geo.status}</div>
              </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
               ...
               <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                  ...
                  <input 
                     ...
                     onChange={(e) => handleAutoSave(setForecastHorizon, "forecastHorizon", parseInt(e.target.value))}
               ...
               <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                  ...
                  <input 
                     ...
                     onChange={(e) => handleAutoSave(setDiscountThreshold, "discountThreshold", parseInt(e.target.value))}
               ...
               <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                  ...
                  <input 
                     ...
                     value={rfmAtRisk} 
                     onChange={(e) => handleAutoSave(setRfmAtRisk, "rfmAtRisk", parseInt(e.target.value))}
      ...
      {/* 4. EXPORT & REPORTING CONFIGURATION */}
      <Card className="w-full rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl">
         <CardHeader className="border-b border-slate-800/50 pb-4 mb-4">
            ...
            <div className="grid gap-6 md:grid-cols-2">
               ...
               <div className="bg-slate-900/50 border border-slate-800 p-5 rounded-2xl flex flex-col gap-5">
                  <div>
                     ...
                     <div className="flex flex-wrap gap-2">
                        <Badge 
                           className={`px-3 py-1.5 cursor-pointer transition-colors ${exportFormat === "html" ? "bg-sky-500/20 text-sky-400 border border-sky-500/30 hover:bg-sky-500/30" : "bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-700"}`}
                           onClick={() => handleAutoSave(setExportFormat, "exportFormat", "html")}
                        >
                           HTML Executive
                        </Badge>
                        <Badge 
                           className={`px-3 py-1.5 cursor-pointer transition-colors ${exportFormat === "csv" ? "bg-sky-500/20 text-sky-400 border border-sky-500/30 hover:bg-sky-500/30" : "bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-700"}`}
                           onClick={() => handleAutoSave(setExportFormat, "exportFormat", "csv")}
                        >
                           Raw CSV
                        </Badge>
                        <Badge 
                           className={`px-3 py-1.5 cursor-pointer transition-colors ${exportFormat === "json" ? "bg-sky-500/20 text-sky-400 border border-sky-500/30 hover:bg-sky-500/30" : "bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-700"}`}
                           onClick={() => handleAutoSave(setExportFormat, "exportFormat", "json")}
                        >
                           JSON Object
                        </Badge>
                     </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-slate-800/80">
                     <Button 
                       className="w-full bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 transition-colors"
                       onClick={handleTestDownload}
                     >
                        <Download className="h-4 w-4 mr-2" /> 
                        Test Download ({exportFormat.toUpperCase()})
                     </Button>
                  </div>
               </div>
               ...
               <div className="bg-slate-900/50 border border-slate-800 p-5 rounded-2xl flex flex-col gap-5">
                  <div>
                     ...
                     <div className="flex flex-wrap gap-2">
                        <Badge 
                           className={`px-3 py-1.5 cursor-pointer transition-colors ${cacheTTL === "1h" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30" : "bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-700"}`}
                           onClick={() => handleAutoSave(setCacheTTL, "cacheTTL", "1h")}
                        >
                           1 Hour
                        </Badge>
                        <Badge 
                           className={`px-3 py-1.5 cursor-pointer transition-colors ${cacheTTL === "24h" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30" : "bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-700"}`}
                           onClick={() => handleAutoSave(setCacheTTL, "cacheTTL", "24h")}
                        >
                           24 Hours
                        </Badge>
                        <Badge 
                           className={`px-3 py-1.5 cursor-pointer transition-colors ${cacheTTL === "live" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30" : "bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-700"}`}
                           onClick={() => handleAutoSave(setCacheTTL, "cacheTTL", "live")}
                        >
                           No Cache (Live)
                        </Badge>
                     </div>
                  ...
                  {/* ACTIVE BACKGROUND WORKER PROOF */}
                  <div className="mt-auto pt-4 border-t border-slate-800/80">
      ...
      {/* 5. DANGER ZONE */}
      <Card className="w-full rounded-3xl border border-rose-500/20 bg-rose-950/10 shadow-2xl mt-4">
         <CardHeader>
         ...
         <CardContent>
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between bg-slate-950 border border-rose-900/30 p-5 rounded-2xl">
               ...
               <Button 
                  className={`mt-4 md:mt-0 transition-colors ${clearStatus ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500 hover:text-white" : "bg-rose-500/10 text-rose-400 border-rose-500/20 hover:bg-rose-500 hover:text-white"}`}
