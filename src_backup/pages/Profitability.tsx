  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 shadow-xl">
        <p className="font-semibold text-white mb-2">{data.period}</p>
        <p className="text-sm text-sky-400 mb-1">Sales: {moneyExact(data.sales)}</p>
        <p className={`text-sm mb-1 ${data.profit >= 0 ? "text-emerald-400" : "text-rose-400"}`}>Profit: {moneyExact(data.profit)}</p>
        <p className={`text-sm font-medium pt-1 border-t border-slate-800 ${color}`}>Margin: {pct(data.margin)}</p>
      </div>
    );
  }
  return null;
...
const GrowthTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 shadow-xl">
        <p className="font-semibold text-white mb-2">{data.period}</p>
        <p className={`text-sm font-medium ${color}`}>MoM Profit Growth: {pct(data.growth)}</p>
      </div>
    );
  }
  return null;
...
const CustomCategoryProfitTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    const data = payload[0].payload;
    ...
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 shadow-xl">
        ...
        <p className="text-sm text-amber-400 mb-1">Orders: {formatNumber(data.orders)}</p>
        <p className="text-sm text-sky-400 mb-1">Sales: {moneyExact(data.sales)}</p>
        <p className="text-sm text-emerald-400 mb-1">Net Profit: {moneyExact(data.profit)}</p>
        <p className="text-sm text-purple-400 font-medium pt-1 border-t border-slate-800">Profit Share: {pct(profitPct)}</p>
      </div>
    );
  }
  return null;
...
const CustomSubCategoryProfitTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    const data = payload[0].payload;
    const profitPct = (data.profit / TOTAL_PROFIT) * 100;
    return (
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-4 shadow-xl">
        <p className="font-semibold text-white mb-2">{data.name}</p>
        <p className="text-sm text-amber-400 mb-1">Orders: {formatNumber(data.orders)}</p>
        <p className={`text-sm mb-1 ${data.profit >= 0 ? "text-emerald-400" : "text-rose-400"}`}>Net Profit: {moneyExact(data.profit)}</p>
        <p className={`text-sm font-medium pt-1 border-t border-slate-800 ${profitPct >= 0 ? "text-purple-400" : "text-rose-400"}`}>
          Profit Share: {pct(profitPct)}
        </p>
      </div>
    );
  }
        ...
        <CardContent className="p-5 flex flex-col h-full justify-between">
          ...
          <div className="mt-5 flex items-center justify-between">
            <div className={`flex items-center gap-1 text-sm font-medium ${up ? "text-emerald-400" : "text-rose-400"}`}>
              ...
              <span>{up ? "Positive" : "Negative"} Status</span>
            </div>
          </div>
        </CardContent>
      </Card>
...
export default function Profitability() {
  ...
  return (
    <div className="mx-auto mt-6 flex flex-col min-w-0 max-w-[1600px] gap-6 w-full">
      ...
      <Card className="min-w-0 rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardContent className="flex flex-col gap-4 p-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex items-start gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 shadow-lg shadow-emerald-500/20 shrink-0">
              <TrendingUp className="h-6 w-6 text-white" />
            </div>
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-[0.22em] text-slate-500">Business Diagnostics</p>
              <h2 className="text-xl font-semibold text-white">Profitability Overview</h2>
              <p className="mt-1 max-w-3xl text-sm text-slate-400">
                End-to-End full width analysis of profit margins, loss distribution, categorical efficiency, and root cause leakage.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      {/* 2. Key Profitability KPIs */}
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 w-full items-stretch">
        <MetricCard title="Total Profit" value="$1.47M" detail="All-time retained earnings" trend="up" icon={Wallet} />
        <MetricCard title="Profit Margin" value="11.61%" detail="Overall net profitability" trend="up" icon={Percent} />
        <MetricCard title="Profit Orders" value={formatNumber(38746)} detail="Orders making positive money" trend="up" icon={BadgeDollarSign} />
        <MetricCard title="Loss Orders" value={formatNumber(12544)} detail="Orders executed at a loss" trend="down" icon={AlertOctagon} />
        <MetricCard title="Top Sub-Category" value="24.24%" detail="Paper (Highest Margin)" trend="up" icon={Activity} />
        <MetricCard title="Worst Discount" value="-111%" detail="Margin on 50%+ clearance" trend="down" icon={ShieldAlert} />
      </section>

      {/* 3. Profit vs Loss Order Distribution (Pie Chart) */}
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Profit vs Loss Distribution" subtitle="Proportion of orders generating profit versus operating at a loss" />
        </CardHeader>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie 
                ...
                labelLine={false}
              >
                {profitLossDist.map((entry, index) => (
      ...
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Monthly Profit Trend" subtitle="Net profit fluctuation over the multi-year tracking period" />
        </CardHeader>
        <CardContent className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={monthlyData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <YAxis stroke="#94a3b8" tickLine={false} axisLine={false} tickFormatter={(val) => money(val)} />
              <Tooltip 
                contentStyle={{ background: "#020617", border: "1px solid #1f2937", borderRadius: 16, color: "#e2e8f0" }} 
                formatter={(value) => moneyExact(Number(value)) }
              />
              <Area type="monotone" dataKey="profit" name="Net Profit" stroke="#34d399" fill="#34d399" fillOpacity={0.15} strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>

      {/* 5. Sales & Profit Dual Trend */}
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Sales & Profit Dual Trend" subtitle="Mapping topline revenue against bottom-line profitability" />
        </CardHeader>
        <CardContent className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={monthlyData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <Bar dataKey="profit" name="Net Profit" barSize={12} fill="#34d399" opacity={0.6} />
            </ComposedChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
      ...
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Monthly Profit Margin (%)" subtitle="Efficiency health tracking representing margin percent scaling" />
        </CardHeader>
        <CardContent className="h-[350px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={monthlyData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <ReferenceLine y={10} stroke="#cbd5e1" strokeDasharray="3 3" label={{ position: 'insideTopLeft', value: '10% Benchmark', fill: '#cbd5e1', fontSize: 12 }} />
              <Line type="monotone" dataKey="margin" name="Profit Margin" stroke="#f59e0b" strokeWidth={3} dot={false} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>

      {/* 7. Monthly Growth (MoM) by Profits */}
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Profit Growth (Month-over-Month %)" subtitle="Relative percentage change in net profit compared to the immediate prior month" />
        </CardHeader>
        <CardContent className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={monthlyData.slice(1)} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <Bar dataKey="growth" name="MoM Profit Growth" radius={[2, 2, 0, 0]}>
      ...
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Discount Impact on Profitability" subtitle="Profit margins collapse exponentially at discounts of 20% or higher" />
        </CardHeader>
        <CardContent className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={discountData} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
              <XAxis dataKey="band" stroke="#94a3b8" tickLine={false} axisLine={false} />
              <YAxis stroke="#94a3b8" tickLine={false} axisLine={false} tickFormatter={(val) => money(val)} />
              <Tooltip 
                contentStyle={{ background: "#020617", border: "1px solid #1f2937", borderRadius: 16, color: "#e2e8f0" }}
                formatter={(value, name) => name === "Margin %" ? pct(Number(value)) : moneyExact(Number(value))} 
              />
              <ReferenceLine y={0} stroke="#cbd5e1" strokeWidth={2} />
              <Bar dataKey="profit" name="Net Profit" radius={[4, 4, 4, 4]}>
                {discountData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.profit >= 0 ? "#34d399" : "#fb7185"} opacity={0.7} />
                ))}
              </Bar>
              <Line type="monotone" dataKey="margin" name="Margin %" stroke="#f59e0b" strokeWidth={3} dot={false} />
            </ComposedChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>

      {/* 9. Peak & Lowest Performance Highlight */}
      <div className="grid gap-6 md:grid-cols-2 w-full">
        <Card className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 shadow-2xl shadow-slate-950/40">
          <CardContent className="p-6">
            ...
            <h3 className="mt-2 text-2xl font-semibold text-white">Best Profit Margin Month</h3>
            <p className="mt-2 text-xl text-emerald-300 font-medium">{peakMonth.period}</p>
            <div className="mt-2 flex gap-4 text-sm text-slate-300">
               <p>Margin: <span className="font-semibold text-emerald-400">{pct(peakMonth.margin)}</span></p>
               <p>Profit: <span className="font-semibold text-white">{moneyExact(peakMonth.profit)}</span></p>
            </div>
          </CardContent>
        </Card>
        <Card className="rounded-3xl border border-rose-500/20 bg-rose-500/10 shadow-2xl shadow-slate-950/40">
          <CardContent className="p-6">
            ...
            <h3 className="mt-2 text-2xl font-semibold text-white">Worst Profit Margin Month</h3>
            <p className="mt-2 text-xl text-rose-300 font-medium">{lowestMonth.period}</p>
            <div className="mt-2 flex gap-4 text-sm text-slate-300">
               <p>Margin: <span className="font-semibold text-rose-400">{pct(lowestMonth.margin)}</span></p>
               <p>Profit: <span className="font-semibold text-white">{moneyExact(lowestMonth.profit)}</span></p>
            </div>
          </CardContent>
        </Card>
      </div>

      {/* 10. Profit by Category (Donut Chart) */}
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Profit by Primary Category" subtitle="Total absolute profitability mapped across core verticals" />
        </CardHeader>
        <CardContent className="h-[400px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip content={<CustomCategoryProfitTooltip />} />
              <Legend verticalAlign="bottom" height={36} />
            </PieChart>
          </ResponsiveContainer>
        </CardContent>
      ...
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Profit by Sub-Category" subtitle="A granular breakdown of profitability across all product segments" />
        </CardHeader>
        <CardContent className="h-[450px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={subCategoryData} margin={{ top: 20, right: 20, bottom: 60, left: 20 }}>
              ...
              <YAxis stroke="#94a3b8" tickLine={false} axisLine={false} tickFormatter={(val) => money(val)} />
              <Tooltip cursor={{ fill: "#1e293b" }} content={<CustomSubCategoryProfitTooltip />} />
              <ReferenceLine y={0} stroke="#cbd5e1" strokeWidth={2} />
              <Bar dataKey="profit" name="Net Profit" radius={[4, 4, 4, 4]}>
      ...
      {/* 12. Profitability Efficiency Matrix */}
      <Card className="min-w-0 overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Profitability Efficiency Matrix" subtitle="Sub-Categories mapped into 5 diagnostic segments based on absolute sales vs margin percentage" />
        </CardHeader>
        <CardContent className="h-[500px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" />
              <XAxis type="number" dataKey="sales" name="Sales" stroke="#94a3b8" tickFormatter={(val) => money(val)} />
              <YAxis type="number" dataKey="margin" name="Profit Margin (%)" stroke="#94a3b8" tickFormatter={(val) => `${val}%`} />
              <ZAxis type="number" dataKey="orders" range={[150, 600]} name="Orders" />
              <Tooltip 
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-3 shadow-xl">
                        ...
                        <p className="text-sm text-sky-400 mt-1">Sales: {moneyExact(data.sales)}</p>
                        <p className={`text-sm mt-0.5 ${data.profit >= 0 ? "text-emerald-400" : "text-rose-400"}`}>Profit Margin: {pct(data.margin)}</p>
                      </div>
                    );
                  }
                  return null;
              ...
              <Scatter name="Segments" data={subCategoryData}>
                {subCategoryData.map((entry, index) => (
      ...
      {/* 13. Strategic Profitability Notes */}
      <Card className="min-w-0 rounded-3xl border border-slate-800 bg-slate-950/80 shadow-2xl shadow-slate-950/40 w-full">
        <CardHeader className="pb-2">
          <SectionTitle title="Strategic Margin Observations" subtitle="Actionable diagnostic notes generated from the profitability root-cause analysis" />
        </CardHeader>
